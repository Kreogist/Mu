...
KNMacExtras::KNMacExtras(QObject *parent) :
    ...
    m_playIcon(QIcon(":/platform/platforms/mac/play.png")),
    m_pauseIcon(QIcon(":/platform/platforms/mac/pause.png"))
{
    //Configure the button icons.
    QIcon actionIcons[ThumbnailActionsCount];
    actionIcons[PlayPrev]=QIcon(":/platform/platforms/mac/prev.png");
    actionIcons[PlayAndPause]=m_playIcon;
    actionIcons[PlayNext]=QIcon(":/platform/platforms/mac/next.png");
    //Initial all the system tray icon button.
    for(int i=ThumbnailActionsCount-1; i>-1; i--)
    {
        //Initial the tray icons.
        m_actionButtons[i]=new QSystemTrayIcon(this);
        //Set the dummy context menu.
