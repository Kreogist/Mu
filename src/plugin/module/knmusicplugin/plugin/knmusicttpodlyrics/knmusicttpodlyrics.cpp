...
{
    m_table="00000000 77073096 EE0E612C 990951BA 076DC419 706AF48F E963A535 "
                                        ...
                                        QList<KNMusicLyricsDetails> &lyricsList)
{
    ...
    for(QJsonArray::iterator i=songListData.begin();
        i!=songListData.end();
        ++i)
    {
        QJsonObject currentObject=(*i).toObject();
        //Generate the URL.
        QString downloadURL="http://lp.music.ttpod.com/lrc/down?artist=" +
                            currentObject.value("singerName").toString() +
                            "&title=" +
                            currentObject.value("songName").toString() +
                            "&code=" +
                            process_code(currentObject.value("neid").toVariant());
...
{
    qint32 ret=crc32(QString::number((qint32)str.toDouble()));
    if(ret==-1)
    {
        return QString();
    }
    ret = ret >> 3;
    QString absRetText=QString::number(qAbs(ret), 16);
    return ret<0?QString("-"+absRetText):absRetText;
}

qint32 KNMusicTTPodLyrics::crc32(const QString &str)
{
    qint32 crc=0, y=0;
    crc^=-1;
    //Ignore the last one, that should be 0.
    for(int i=0; i<str.size(); i++)
    {
        y=(crc^(qint32)str.at(i).unicode()) & 0xFF;
        if(y<0 || y*9>m_table.size())
        {
            return -1;
        }
        crc=((qint32)((quint32)crc>>8))^((qint32)m_table.mid(y*9, 8).toLongLong(0, 16));
    }
    return (crc^(-1));
}
